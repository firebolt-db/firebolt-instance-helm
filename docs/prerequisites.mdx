...
- **Container registry access**. The chart pulls the engine through `oci.firebolt.io` and metadata from `ghcr.io/firebolt-db` by default. To use GHCR directly or mirror through your own registry, see [Image overrides](./usage/image-overrides).
