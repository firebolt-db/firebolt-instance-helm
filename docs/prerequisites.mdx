...
- **An engine image with `instance.auth` support**. Required only if you set `auth.enabled` or `tls.engine.enabled`. The chart's pinned `appVersion` predates that config schema; an engine on that build crash-loops instead of starting. See [Authentication and TLS](./usage/authentication).
