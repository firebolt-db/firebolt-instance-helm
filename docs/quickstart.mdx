...
The `oci.firebolt.io` host is Firebolt's [Scarf](https://scarf.sh) gateway. It records the requested chart version and platform and redirects to GHCR; it does not receive chart values, application data, query data, schemas, secrets, or configuration. As with any network request, the source IP is visible to Scarf; Scarf may use it to infer the company and does not store it. To bypass the gateway, pull from `oci://ghcr.io/firebolt-db/helm-charts/firebolt-instance` instead.
