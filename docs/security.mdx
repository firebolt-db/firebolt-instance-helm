...
`tls.gateway.enabled` and `tls.engine.enabled` change what's carried on the "External clients → Gateway" and "Gateway → Engine pods" flows above from plaintext HTTP to TLS; they don't change which flows are required. See [Authentication and TLS](./usage/authentication).
...
- [Authentication and TLS](./usage/authentication): enabling engine authentication, JWT signing key rotation, and TLS on the gateway and engine listeners.
