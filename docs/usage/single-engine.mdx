---
description: Default install of the firebolt-instance Helm chart with one engine, one gateway, the Metadata Service, and bundled PostgreSQL.
...
<Note>
Engines need object storage configured under `customEngineConfig.storage`. Without it, the engine pod never becomes Ready. Configure it first ([Managed storage](./managed-storage)).
</Note>
