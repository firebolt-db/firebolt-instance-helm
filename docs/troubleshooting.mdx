...
`customEngineConfig.storage` is unset. Without object storage the engine does not enter the Ready state. Configure object storage and run `helm upgrade`. See [Object Storage](./usage/object-storage/amazon-s3).
