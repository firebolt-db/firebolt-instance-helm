...
│   ├── dependabot.yml        # github-actions (grouped into one PR) + docker updates
│   └── ISSUE_TEMPLATE/
├── static/                   # chart icon
...
- **GitHub Actions are pinned to commit SHAs.** All `uses:` references in `.github/workflows/*` use full 40-char SHAs with a version comment. `actionlint` and `zizmor` run on every push/PR that touches `.github/`. New actions you introduce MUST follow the same pinning convention. Dependabot keeps those SHAs current and bundles every action bump into a single `chore(deps):` PR (the `actions` group in `.github/dependabot.yml`, with a 7-day cooldown that internal `firebolt-analytics/*` actions are exempt from), so an action upgrade is reviewed as one changeset rather than one PR per action.
