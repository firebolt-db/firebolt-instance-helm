...
│   ├── values-dev.yaml       # committed dev overlay used by `make dev` (ECR + :dev + floci storage)
...
- `make install` — plain `helm install` of the chart with its defaults. No overlay, no ECR pull secret, no floci. Useful for verifying chart behaviour against unmodified inputs.
