...
- Symptom: engine pods can fail during startup if the chart moves `--data-dir` or the `data` volume away from `/var/lib/firebolt`. Cause: the engine treats `/var/lib/firebolt` as its writable runtime data directory. Resolution: keep the `data` mount, `--data-dir`, `config.yaml`, `auth.json`, and memlock PID files under `/var/lib/firebolt`, and keep the writable `runtime` emptyDir mounted at `/run/firebolt` (the engine's per-port unix-domain socket dir).
- Symptom: the engine binary is invoked as bare `firebolt` (on `PATH`) rather than an absolute path. Cause: the firebolt-core image layout changed in FB-1733 (packdb PR #23532) — the read-only app payload moved to `/opt/firebolt` (symlinked onto `PATH` as `/usr/local/bin/firebolt`), the writable data root moved from `/firebolt-core/volume` to `/var/lib/firebolt` (dropping the `volume/` nesting), the socket dir moved to `/run/firebolt`, and the legacy `/firebolt-core/firebolt` path plus the `--node` CLI flag were removed (node identity now comes from the `FIREBOLT_CORE_NODE` env). The `packdb` ECR image places the binary at `/usr/bin/firebolt` instead of `/opt/firebolt`, so the chart resolves it via `PATH` to stay correct for both image layouts. Resolution: invoke `firebolt server start --data-dir /var/lib/firebolt`; do not hard-code an absolute binary path.
